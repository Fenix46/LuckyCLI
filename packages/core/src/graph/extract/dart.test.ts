...
async function extract(path: string, source: string) {
  const parsed = await parse("dart", source);
  try {
    return dartExtractor.extract({ path, source, root: parsed.root });
  } finally {
    parsed.dispose();
  }
}
...
describe("dart extractor", () => {
  it("extracts a self-consistent graph", async () => {
    const { nodes, edges } = await extract("rect.dart", SAMPLE);
    expect(validateGraph({ meta: anyMeta(), nodes, edges })).toEqual([]);
  });

  it("emits file, abstract class (interface), class, method and function nodes", async () => {
    const { nodes } = await extract("rect.dart", SAMPLE);
    expect(byLabel(nodes, "rect.dart")[0]?.kind).toBe("file");
  ...
  it("emits imports edges for import directives", async () => {
    const { nodes, edges } = await extract("rect.dart", SAMPLE);
    expect(byLabel(nodes, "package:flutter/material.dart")[0]?.kind).toBe("module");
    expect(byLabel(nodes, "dart:math")[0]?.kind).toBe("module");
